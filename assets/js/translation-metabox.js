/**
 * Vaani Translations meta box — Translate-now buttons.
 *
 * Queues a background translation over admin-ajax.php and swaps the matching
 * language row's status cell with the HTML returned by the server.
 */
document.addEventListener( 'DOMContentLoaded', () => {
	const config = window.vaaniTranslate;
	const box = document.getElementById( 'vaani_translations' );
	...
	box.addEventListener( 'click', async ( event ) => {
		const button = event.target.closest( '.vaani-translate' );
		if ( ! button ) {
			return;
		}
